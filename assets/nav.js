...
(function () {
  ...
  var STYLE = [
    ...
    // Collapsed is decided by measurement, not by a width guess: the bar
    // collapses as soon as the links stop fitting, so no item is ever hidden
    // in the horizontally scrolling nav.
    ".topbar[data-collapsed] .nav-toggle{",
    ...
    ".topbar[data-collapsed] .nav-toggle span{display:block;height:1.5px;width:100%;border-radius:2px;",
      "background:var(--text-soft,#7A6266);transition:transform .2s ease,opacity .2s ease}",
    ".topbar[data-collapsed] .nav-toggle:hover{border-color:var(--accent,#C9A877)}",
    ".topbar[data-collapsed] .nav-toggle:focus-visible{outline:2px solid var(--accent,#C9A877);outline-offset:2px}",
    ".topbar[data-collapsed] .nav-toggle[aria-expanded='true'] span:nth-child(1){transform:translateY(5.5px) rotate(45deg)}",
    ".topbar[data-collapsed] .nav-toggle[aria-expanded='true'] span:nth-child(2){opacity:0}",
    ".topbar[data-collapsed] .nav-toggle[aria-expanded='true'] span:nth-child(3){transform:translateY(-5.5px) rotate(-45deg)}",
    ".topbar[data-collapsed] .nav-panel{",
    ...
    ".topbar[data-collapsed] .nav-panel[data-open]{display:flex}",
    ".topbar[data-collapsed] .nav-panel .navlinks{flex-direction:column;gap:0.45rem;overflow:visible}",
    ".topbar[data-collapsed] .nav-panel .navlinks a{text-align:center;padding:0.62rem 0.95rem}",
    ".topbar[data-collapsed] .nav-panel .lang-switch{align-self:center;margin-top:0.2rem}"
  ].join("");
  ...
  function init() {
    ...
    // Collapse exactly when the links stop fitting on one line. Measuring beats
    // a breakpoint: the menu grows with the sheet, and German labels are wider.
    function updateCollapsed() {
      var wasOpen = panel.hasAttribute("data-open");
      bar.removeAttribute("data-collapsed");
      var overflows = links.scrollWidth > links.clientWidth + 1 ||
                      bar.scrollWidth > bar.clientWidth + 1;
      if (overflows) {
        bar.setAttribute("data-collapsed", "");
        if (wasOpen) panel.setAttribute("data-open", "");
      } else {
        setOpen(false);                       // inline again: never leave it open
      }
    }
    ...
    if (document.fonts && document.fonts.ready && document.fonts.ready.then) {
      document.fonts.ready.then(updateCollapsed);
    }
